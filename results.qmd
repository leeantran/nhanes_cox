...
med_covs <- c("RIDAGEYR", "RIAGENDR", "RIDRETH1", "diabetes", "hyperten")
...
draw_mediation_plot <- function(event_var, title_str) {
  ...
  se_a <- sqrt(vcov(fit_path_a)["TyHGB", "TyHGB"])
  ...
  beta_b <- coef(fit_dir)[med_row]
  hr_b <- sm_dir$conf.int[med_row, "exp(coef)"]
  p_b <- sm_dir$coefficients[med_row, "Pr(>|z|)"]
  
  # FIX: TÍNH LẠI PHẦN TRĂM BẰNG PHƯƠNG PHÁP SOBEL (Khớp 100% với Bảng 14)
  ab <- beta_a * beta_b
  prop_med <- (ab / beta_tot) * 100
...
#| fig-cap: "Supplemental Figure: Stability of the TyHGB prognostic value across multiple imputed datasets (m=5)"
#| fig-width: 14
#| fig-height: 6
...
get_mice_hr <- function(event_var, title) {
  res_df <- data.frame()
  estimates <- c(); vars <- c()
  ...
  for(i in 1:5) {
    ...
    d_i$diabetes <- as.numeric(as.character(d_i$diabetes)); d_i$hyperten <- as.numeric(as.character(d_i$hyperten))
    
    des_i <- svydesign(id=~SDMVPSU, strata=~SDMVSTRA, weights=~WTSAF2YR, nest=TRUE, data=d_i)
    fit_i <- svycoxph(as.formula(paste("Surv(time_months, ", event_var, ") ~ TyHGB +", covs_str)), design = des_i)
    
    estimates[i] <- coef(fit_i)["TyHGB"]
    vars[i] <- vcov(fit_i)["TyHGB", "TyHGB"]
  ...
  res_df <- rbind(res_df, data.frame(Imputation = "Pooled", HR = exp(Q_bar), 
  ...
  ggplot(res_df, aes(x = HR, y = Imputation)) +
    ...
    geom_errorbarh(aes(xmin = LCI, xmax = UCI), height = 0.2, color = "#0055A4", linewidth = 1) +
    geom_point(size = 4, color = ifelse(res_df$Imputation == "Pooled", "#E64B35", "#0055A4"), 
               shape = ifelse(res_df$Imputation == "Pooled", 18, 16)) +
    theme_classic() + labs(title = title, x = "Hazard Ratio (95% CI)", y = "") +
    theme(plot.title = element_text(face = "bold"), axis.text = element_text(size = 11, color="black", face="bold"))
}

p_mice_acm <- get_mice_hr("status_acm", "A. ACM Models")
p_mice_cvm <- get_mice_hr("status_cvm", "B. CVM Models")
...
```{r}
#| label: fig-evalue
#| fig-cap: "Supplemental Figure: E-value curves representing the minimum strength of an unmeasured confounder required to nullify the TyHGB effect"
#| fig-width: 14
#| fig-height: 6
...
# TÍNH TOÁN THỦ CÔNG E-VALUE (Chuẩn hóa theo VanderWeele & Ding 2017)
...
plot_evalue <- function(event_var, title) {
  
  # 1. Chạy mô hình
  form <- as.formula(paste("Surv(time_months, ", event_var, ") ~ TyHGB +", covs_str))
  fit <- svycoxph(form, design = masld_design)
  
  # 2. Lấy số liệu gốc từ Summary
  sm <- summary(fit)
  hr <- as.numeric(sm$conf.int["TyHGB", 1])
  lci <- as.numeric(sm$conf.int["TyHGB", 3])
  
  # 3. Tính toán bằng công thức gốc
  e_point <- get_evalue(hr)
  e_lower <- get_evalue_ci(hr, lci)
  
  # 4. MÔ PHỎNG HÀM SỐ TOÁN HỌC
  rr_eu <- seq(1.001, 20, length.out = 1000) 
  
  # Tính y (RR_UD) tương ứng
  ...
  # BẢNG MÀU MỚI: Sang trọng, chuẩn tạp chí The Lancet (Deep Rose & Deep Teal)
  col_point <- "#C2185B"  # Đỏ tía 
  col_lower <- "#008080"  # Xanh cổ vịt 
  
  # Tạo "khung cửa sổ" zoom
  axis_limit <- max(e_point * 2.2, e_lower * 3, 3.5) 
  ...
  if (e_lower > 1) {
    ...
    p <- p + geom_line(aes(y = rr_ud_lower, color = "Lower Confidence Limit"), linewidth = 1.5, linetype = "solid", na.rm = TRUE) +
      ...
      # ÉP CHỮ LCI NẰM PHÍA DƯỚI (GẦN 0) BẰNG vjust = 1
  ...
  p <- p + geom_point(aes(x = e_point, y = e_point), color = col_point, size = 4.5) +
    # ÉP CHỮ E-VALUE NẰM PHÍA TRÊN BẰNG vjust = 0
    ...
    # Cửa sổ hiển thị cắt mượt mà mọi đường thừa
    ...
    # THEME CLASSIC (Trắng toát giống hình thầy)
    theme_classic() + 
...
p_ev_acm <- plot_evalue("status_acm", "A. All-Cause Mortality")
p_ev_cvm <- plot_evalue("status_cvm", "B. Cardiovascular Mortality")
