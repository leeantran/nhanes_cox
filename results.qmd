...
pacman::p_load(readxl, survey, survival, gtsummary, survminer, splines, mice, timeROC, 
               ...
               patchwork, gt, forestploter, regplot)
...
df_fg <- df_final[, c("time_months", "event_cr", "SDMVPSU", "SDMVSTRA", "WTSAF2YR", 
                      "TyHGB_Z", "ABSI_Z", "WWI_Z", "WHtR_Z", "RIDAGEYR", "LBXSATSI")]
...
# ==============================================================================
# BƯỚC 10: VẼ NOMOGRAM (Loại bỏ đa cộng tuyến & Tách riêng thang đo xác suất)
# ==============================================================================
library(rms)
library(survival)
library(dplyr)

# 1. Khai báo 5 biến độc lập (ĐÃ BỎ Glucose, HDL, Triglyceride để chống Đa cộng tuyến)
quant_vars <- c("TyHGB", "RIDAGEYR", "LBXWBCSI", "LBXSATSI", "LBXSASSI")

sys_vars <- c("time_months", "status_acm", "status_cvm", "event_cr", "WTSAF2YR")

# 2. CHỌN ĐÚNG CỘT RỒI MỚI XÓA NA
df_nomo_rms <- df_final %>%
  select(all_of(c(sys_vars, quant_vars))) %>% 
  drop_na()
...
# ------------------------------------------------------------------------------
# 3. XÂY DỰNG MÔ HÌNH VÀ VẼ CHO ACM
# ------------------------------------------------------------------------------
form_acm <- as.formula(paste("Surv(time_months, status_acm) ~", paste(quant_vars, collapse = " + ")))

fit_acm_nomo <- cph(form_acm, data = df_nomo_rms, x = TRUE, y = TRUE, surv = TRUE, weights = df_nomo_rms$WTSAF2YR)
...
# Thang đo ACM: Tử vong chung cao hơn, rải từ 0.95 xuống 0.5 (khoảng cách thưa ra)
prob_scale_acm <- c(0.95, 0.90, 0.80, 0.70, 0.60, 0.50)

nom_acm <- nomogram(fit_acm_nomo, 
                    fun = list(surv3_acm, surv5_acm, surv10_acm),
                    funlabel = c("3-Year ACM Survival", "5-Year ACM Survival", "10-Year ACM Survival"),
                    fun.at = prob_scale_acm,
                    lp = FALSE)

# ------------------------------------------------------------------------------
# 4. XÂY DỰNG MÔ HÌNH VÀ VẼ CHO CVM (FINE-GRAY)
# ------------------------------------------------------------------------------
...
# Cập nhật datadist cho bộ data Fine-Gray
dd_fg <- datadist(fg_data_nomo)
options(datadist = "dd_fg")

form_cvm <- as.formula(paste("Surv(fgstart, fgstop, fgstatus) ~", paste(quant_vars, collapse = " + ")))

fit_cvm_nomo <- cph(form_cvm, data = fg_data_nomo, x = TRUE, y = TRUE, surv = TRUE, weights = fg_data_nomo$final_wt)
...
# Thang đo CVM: Tử vong tim mạch thấp hơn, bỏ 0.85 để chữ không cắn nhau
prob_scale_cvm <- c(0.99, 0.95, 0.90, 0.80, 0.70)

nom_cvm <- nomogram(fit_cvm_nomo, 
                    fun = list(surv3_cvm, surv5_cvm, surv10_cvm),
                    funlabel = c("3-Year CVM Survival", "5-Year CVM Survival", "10-Year CVM Survival"),
                    fun.at = prob_scale_cvm,
                    lp = FALSE)
...
plot(nom_acm, cex.axis=0.8, xfrac=0.45)
title("A. Nomogram for All-Cause Mortality (ACM)", adj = 0, line = 1, cex.main=1.2)

plot(nom_cvm, cex.axis=0.8, xfrac=0.45)
