---
title: "Prognostic Value of TyHGB Index in Patients with MASLD"
author: An Tran
date: last-modified
format:
  docx:
    fig-dpi: 300
    fig-cap-location: "bottom"
    tbl-cap-location: "top"
execute:
  echo: false
  warning: false
  message: false
knitr:
  opts_chunk: 
    fig.path: "Figures/"
    dev: "jpeg"             
---

```{r}
pacman::p_load(readxl, survey, survival, gtsummary, survminer, splines, mice, timeROC, 
               dplyr, ggplot2, ggsci, rms, tidycmprsk, tidyr, survIDINRI, tableone,
               patchwork, gt, forestploter, regplot, dcurves)

raw_df <- read_excel("data.xlsx", sheet = "RAW DATA")

df_step1 <- raw_df %>% 
  filter(RIDAGEYR >= 20) %>%
  filter(
    !is.na(LBXTR) & !is.na(LBDSGLSI) & !is.na(LBDHDD) & 
      !is.na(LBXSATSI) & !is.na(LBXSASSI) &               
      !is.na(BMXBMI) & !is.na(BMXWAIST) & !is.na(BMXWT) & !is.na(BMXHT) & 
      !is.na(WTSAF2YR) & WTSAF2YR > 0 # Lọc luôn trọng số 0 để tránh warning
  )

df_step1 <- df_step1 %>%
  mutate(cvd_hx = ifelse(MCQ160B == 1 | MCQ160C == 1 | MCQ160E == 1 | MCQ160F == 1, 1, 0))

vars_to_impute <- c("INDFMPIR", "DMDEDUC2", "DMDMARTL", "LBXWBCSI", "LBXSCR", 
                    "LBXSUA", "cvd_hx", "diabetes", "hyperten")

cat_vars <- c("RIAGENDR", "RIDRETH1", "DMDEDUC2", "DMDMARTL", "diabetes", "hyperten", "cvd_hx")
df_step1[cat_vars] <- lapply(df_step1[cat_vars], as.factor)

imputed_data <- mice(df_step1[, vars_to_impute], m = 1, method = 'pmm', seed = 123, printFlag = FALSE)

df_imputed <- df_step1
df_imputed[, vars_to_impute] <- complete(imputed_data, 1)

df_imputed$diabetes <- as.numeric(as.character(df_imputed$diabetes))
df_imputed$hyperten <- as.numeric(as.character(df_imputed$hyperten))
```

```{r}
df_final <- df_imputed %>%
  mutate(
    HSI = 8 * (LBXSATSI / LBXSASSI) + BMXBMI + ifelse(RIAGENDR == 2, 2, 0) + ifelse(diabetes == 1, 2, 0),
    steatosis = ifelse(HSI > 36, 1, 0),
    
    cm_risk = ifelse(
      BMXBMI >= 25 | (RIAGENDR == 1 & BMXWAIST >= 94) | (RIAGENDR == 2 & BMXWAIST >= 80) | 
        LBDSGLSI >= 5.55 | diabetes == 1 | hyperten == 1 | LBXTR >= 150 | 
        (RIAGENDR == 1 & LBDHDD < 40) | (RIAGENDR == 2 & LBDHDD < 50), 1, 0),
    
    is_masld = ifelse(steatosis == 1 & cm_risk == 1, 1, 0),
    
    TyHGB = (LBXTR / LBDHDD) + (0.7 * LBDSGLSI) + (0.1 * BMXBMI),
    ABSI = BMXWAIST / ((BMXBMI^(2/3)) * (BMXHT^(1/2))),
    WWI = BMXWAIST / sqrt(BMXWT),
    WHtR = BMXWAIST / BMXHT,
    
    status_acm = ifelse(mortstat == 1, 1, 0),
    
    status_cvm = ifelse(mortstat == 1 & ucod_leading %in% c("001", "005"), 1, 0),
    time_months = permth_int
  ) %>%
  filter(is_masld == 1, (is.na(RIDEXPRG) | RIDEXPRG != 1), !is.na(mortstat), !is.na(time_months), eligstat == 1) %>%
  mutate(
    TyHGB_Q = factor(ntile(TyHGB, 4), labels = c("Q1", "Q2", "Q3", "Q4")),

    status_acm_factor = factor(status_acm),
    status_cvm_factor = factor(status_cvm),
    
    event_cr = case_when(mortstat == 0 ~ 0, status_cvm == 1 ~ 1, mortstat == 1 & status_cvm == 0 ~ 2)
  )

df_final <- df_final %>%
  mutate(
    DMDEDUC2 = case_when(
      DMDEDUC2 %in% c(1, 2) ~ "Under High School",
      DMDEDUC2 == 3 ~ "High School to University",
      DMDEDUC2 %in% c(4, 5) ~ "Above University",
      TRUE ~ NA_character_
    )
  ) %>%
  drop_na(DMDEDUC2)

df_final <- df_final %>%
  mutate(
   
    DMDMARTL = case_when(
      DMDMARTL %in% c(1, 6) ~ "Married/Partner", 
      DMDMARTL %in% c(2, 3, 4) ~ "Widowed/Divorced/Separated",
      DMDMARTL == 5 ~ "Never married", 
      TRUE ~ NA_character_
    ),
  
    RIDRETH1 = case_when(
      RIDRETH1 == 3 ~ "Non-Hispanic White",
      RIDRETH1 == 4 ~ "Non-Hispanic Black",
      TRUE ~ "Other Races"
    )
  ) %>%
  drop_na(DMDMARTL, RIDRETH1)

cat_vars_final <- c("RIAGENDR", "RIDRETH1", "DMDEDUC2", "DMDMARTL", "diabetes", "hyperten", "cvd_hx")
df_final <- df_final %>% mutate(across(all_of(cat_vars_final), as.factor))

masld_design <- svydesign(id = ~SDMVPSU, strata = ~SDMVSTRA, weights = ~WTSAF2YR, nest = TRUE, data = df_final)
```

## 1. Baseline characteristics of the study population

```{r}
#| label: tbl-baseline
#| tbl-cap: "Table 1: Baseline characteristics of the study population according to TyHGB quartiles"

vars_table1 <- c("RIDAGEYR", "RIAGENDR", "RIDRETH1", "DMDEDUC2", "DMDMARTL", "INDFMPIR",
                 "BMXBMI", "BMXWAIST", "TyHGB", "ABSI", "WWI", "WHtR", 
                 "LBXTC", "LBXTR", "LBDHDD", "LBDSGLSI", "LBXSATSI", "LBXSASSI",
                 "LBXWBCSI", "LBXLYPCT", "LBXMOPCT", "LBXNEPCT", "LBXEOPCT", "LBXBAPCT",
                 "LBDLYMNO", "LBDMONO", "LBDNENO", "LBDEONO", "LBDBANO",
                 "LBXRBCSI", "LBXHGB", "LBXHCT", "LBXWBCSI", "LBDLYMNO", "LBDMONO", "LBDNENO",
                 "LBDEONO", "LBDBANO", "LBXRBCSI", "LBXHGB", "LBXHCT",
                 "diabetes", "hyperten", "cvd_hx", "status_acm_factor", "status_cvm_factor")

tab1_gt <- masld_design %>%
  tbl_svysummary(
    by = TyHGB_Q, 
    include = all_of(vars_table1),
    statistic = list(
      all_continuous() ~ "{mean} ({sd})", 
      all_categorical() ~ "{n_unweighted} ({p}%)"
    ),
    digits = list(
      all_continuous() ~ 2, 
      all_categorical() ~ c(0, 1)
    ), 
    missing = "no",
    
    label = list(
      RIDAGEYR ~ "Age (years)",
      RIAGENDR ~ "Gender",
      RIDRETH1 ~ "Race",
      DMDEDUC2 ~ "Education level",
      DMDMARTL ~ "Marital status",
      INDFMPIR ~ "Family income to poverty",
      
      BMXBMI   ~ "Body mass index (kg/m²)",
      BMXWAIST ~ "Waist circumference (cm)",
      TyHGB    ~ "TyHGB",
      ABSI     ~ "A Body Shape Index (ABSI)",
      WWI      ~ "Weight-Adjusted Waist Index (WWI)",
      WHtR     ~ "Waist-to-Height Ratio (WHtR)",
      
      LBXTC    ~ "Total cholesterol (mg/dL)",
      LBXTR    ~ "Triglycerides (mg/dL)",
      LBDHDD   ~ "HDL-cholesterol (mg/dL)",
      LBDSGLSI ~ "Fasting glucose (mmol/L)",
      LBXSATSI ~ "Alanine aminotransferase (ALT, U/L)",
      LBXSASSI ~ "Aspartate aminotransferase (AST, U/L)",
      
      LBXWBCSI ~ "WBC (1000 cells/μL)",
      LBDLYMNO ~ "Lymphocytes (1000 cells/μL)",
      LBDMONO  ~ "Monocytes (1000 cells/μL)",
      LBDNENO  ~ "Neutrophils (1000 cells/μL)",
      LBDEONO  ~ "Eosinophils (1000 cells/μL)",
      LBDBANO  ~ "Basophils (1000 cells/μL)",
      LBXRBCSI ~ "RBC (million cells/μL)",
      LBXHGB   ~ "Hemoglobin (g/dL)",
      LBXHCT   ~ "Hematocrit (%)",
      
      diabetes ~ "Diabetes",
      hyperten ~ "Hypertension",
      cvd_hx   ~ "History of cardiovascular disease",
      status_acm_factor ~ "All-cause mortality",
      status_cvm_factor ~ "Cardiovascular mortality"
    )
    
  ) %>%
  add_p() %>% 
  add_overall() %>%
  modify_header(
    stat_0 = "**Overall**\n(N = {N_unweighted})", 
    all_stat_cols() ~ "**{level}**\n(N = {n_unweighted})"
  ) %>%
  bold_labels()

tab1_gt
```

## 2. Univariate and multivariate Cox regression

```{r}
#| label: tbl-cox
#| tbl-cap: "Table 2: Univariate and multivariate Cox regression analysis for TyHGB and other indices"


# chuẩn hóa về Z-score (Mean = 0, SD = 1)
df_final <- df_final %>%
  mutate(
    TyHGB_Z = as.numeric(scale(TyHGB)),
    ABSI_Z  = as.numeric(scale(ABSI)),
    WWI_Z   = as.numeric(scale(WWI)),
    WHtR_Z  = as.numeric(scale(WHtR))
  )

masld_design <- svydesign(id = ~SDMVPSU, strata = ~SDMVSTRA, weights = ~WTSAF2YR, nest = TRUE, data = df_final)

markers <- c("TyHGB_Z", "ABSI_Z", "WWI_Z", "WHtR_Z")

adj_vars <- "RIDAGEYR + RIAGENDR + RIDRETH1 + diabetes + hyperten"

get_independent_res <- function(outcome, marker) {
  
  form_uni <- paste0("Surv(time_months, ", outcome, ") ~ ", marker)
  fit_uni <- suppressWarnings(svycoxph(as.formula(form_uni), design = masld_design))
  res_uni <- tidy(fit_uni, exponentiate = TRUE, conf.int = TRUE) %>% filter(term == marker)
  
  hr_uni <- sprintf("%.3f (%.3f-%.3f)", res_uni$estimate, res_uni$conf.low, res_uni$conf.high)
  p_uni <- ifelse(res_uni$p.value < 0.001, "<0.001", sprintf("%.3f", res_uni$p.value))
  
  form_multi <- paste0("Surv(time_months, ", outcome, ") ~ ", marker, " + ", adj_vars)
  fit_multi <- suppressWarnings(svycoxph(as.formula(form_multi), design = masld_design))
  res_multi <- tidy(fit_multi, exponentiate = TRUE, conf.int = TRUE) %>% filter(term == marker)
  
  hr_multi <- sprintf("%.3f (%.3f-%.3f)", res_multi$estimate, res_multi$conf.low, res_multi$conf.high)
  p_multi <- ifelse(res_multi$p.value < 0.001, "<0.001", sprintf("%.3f", res_multi$p.value))
  
  return(c(hr_uni, p_uni, hr_multi, p_multi))
}

results_list <- list()

for (outcome in c("status_acm", "status_cvm")) {
  outcome_label <- ifelse(outcome == "status_acm", "All-cause mortality", "Cardiovascular mortality")
  
  for (marker in markers) {
    res <- get_independent_res(outcome, marker)
    
    clean_marker_name <- gsub("_Z", "", marker)
    
    results_list[[length(results_list) + 1]] <- data.frame(
      Outcome = outcome_label,
      Marker = clean_marker_name,
      Uni_HR = res[1],
      Uni_P = res[2],
      Multi_HR = res[3],
      Multi_P = res[4],
      stringsAsFactors = FALSE
    )
  }
}

df_cox_independent <- bind_rows(results_list)

table_cox_new <- df_cox_independent %>%
  group_by(Outcome) %>%
  gt() %>%
  tab_spanner(label = md("**Univariate analysis (per 1-SD increase)**"), columns = c(Uni_HR, Uni_P)) %>%
  tab_spanner(label = md("**Multivariate Analysis (per 1-SD increase)**"), columns = c(Multi_HR, Multi_P)) %>%
  cols_label(
    Marker = "",
    Uni_HR = md("**HR (95% CIs)**"),
    Uni_P = md("**p-value**"),
    Multi_HR = md("**HR (95% CIs)**"),
    Multi_P = md("**p-value**")
  ) %>%
  tab_options(
    row_group.font.weight = "bold",
    row_group.background.color = "#E8E8E8",
    column_labels.font.weight = "bold",
    table.border.top.color = "black",
    table.border.bottom.color = "black",
    table_body.border.bottom.color = "black"
  ) %>%
  cols_align(align = "left", columns = everything())

table_cox_new
```

## 3. Kaplan–Meier survival curves

```{r}
#| label: fig-km
#| fig-cap: "Figure 1: Kaplan-Meier survival curves for all-cause and cardiovascular mortality"
#| fig-width: 18
#| fig-height: 10

covs_str <- "RIDAGEYR + RIAGENDR + RIDRETH1 + diabetes + hyperten"

cox_acm_m1 <- svycoxph(as.formula(paste("Surv(time_months, status_acm) ~ TyHGB +", covs_str)), design = masld_design)
cox_acm_m2 <- svycoxph(as.formula(paste("Surv(time_months, status_acm) ~ TyHGB + ABSI +", covs_str)), design = masld_design)
cox_acm_m3 <- svycoxph(as.formula(paste("Surv(time_months, status_acm) ~ TyHGB + WWI +", covs_str)), design = masld_design)
cox_acm_m4 <- svycoxph(as.formula(paste("Surv(time_months, status_acm) ~ TyHGB + WHtR +", covs_str)), design = masld_design)

cox_cvm_m1 <- svycoxph(as.formula(paste("Surv(time_months, status_cvm) ~ TyHGB +", covs_str)), design = masld_design)
cox_cvm_m2 <- svycoxph(as.formula(paste("Surv(time_months, status_cvm) ~ TyHGB + ABSI +", covs_str)), design = masld_design)
cox_cvm_m3 <- svycoxph(as.formula(paste("Surv(time_months, status_cvm) ~ TyHGB + WWI +", covs_str)), design = masld_design)
cox_cvm_m4 <- svycoxph(as.formula(paste("Surv(time_months, status_cvm) ~ TyHGB + WHtR +", covs_str)), design = masld_design)

df_final <- df_final %>%
  mutate(
    lp_acm_m1 = predict(cox_acm_m1, type="lp"), Q_acm_m1 = factor(ntile(lp_acm_m1, 4), labels=paste0("Q", 1:4)),
    lp_acm_m2 = predict(cox_acm_m2, type="lp"), Q_acm_m2 = factor(ntile(lp_acm_m2, 4), labels=paste0("Q", 1:4)),
    lp_acm_m3 = predict(cox_acm_m3, type="lp"), Q_acm_m3 = factor(ntile(lp_acm_m3, 4), labels=paste0("Q", 1:4)),
    lp_acm_m4 = predict(cox_acm_m4, type="lp"), Q_acm_m4 = factor(ntile(lp_acm_m4, 4), labels=paste0("Q", 1:4)),
    
    lp_cvm_m1 = predict(cox_cvm_m1, type="lp"), Q_cvm_m1 = factor(ntile(lp_cvm_m1, 4), labels=paste0("Q", 1:4)),
    lp_cvm_m2 = predict(cox_cvm_m2, type="lp"), Q_cvm_m2 = factor(ntile(lp_cvm_m2, 4), labels=paste0("Q", 1:4)),
    lp_cvm_m3 = predict(cox_cvm_m3, type="lp"), Q_cvm_m3 = factor(ntile(lp_cvm_m3, 4), labels=paste0("Q", 1:4)),
    lp_cvm_m4 = predict(cox_cvm_m4, type="lp"), Q_cvm_m4 = factor(ntile(lp_cvm_m4, 4), labels=paste0("Q", 1:4))
  )

masld_design <- svydesign(id = ~SDMVPSU, strata = ~SDMVSTRA, weights = ~WTSAF2YR, nest = TRUE, data = df_final)

create_km <- function(group_var, event_var, model_name) {
  
  form_svy <- as.formula(paste("Surv(time_months,", event_var, ") ~", group_var))
  res_logrank <- svylogrank(form_svy, design = masld_design)
 
  if (!is.null(res_logrank$p.value)) {
    p_val <- res_logrank$p.value
  } else {
    
    chisq_stat <- res_logrank[[2]][1]
    df_stat <- res_logrank[[2]][2]
   
    p_val <- pchisq(chisq_stat, df_stat, lower.tail = FALSE)
  }
  
  p_val <- as.numeric(p_val)
  
 
  if (is.na(p_val)) {
    p_str <- "Weighted P: N/A"
  } else if (p_val < 0.001) {
    p_str <- "Weighted P < 0.001"
  } else {
    p_str <- paste0("Weighted P = ", round(p_val, 3))
  }
  
  fit <- eval(parse(text = paste0("survfit(Surv(time_months, ", event_var, ") ~ ", group_var, ", data = df_final)")))
  
  my_palette <- c("#4daf4a", "#d95f02", "#7570b3", "#984ea3") 
  
  p <- ggsurvplot(
    fit, 
    data = df_final, 
    palette = my_palette, 
    linewidth = 1,
    title = "",                              
    legend.title = model_name,                
    legend.labs = c("Q1", "Q2", "Q3", "Q4"),  
    xlab = "Months", 
    ylab = "Survival Probability",
    pval = p_str,                             
    pval.size = 4,                            
    conf.int = TRUE,                          
    conf.int.style = "ribbon",                
    conf.int.alpha = 0.15,                    
    censor = FALSE,                           
    risk.table = TRUE, 
    risk.table.height = 0.28, 
    fontsize = 3.5, 
    ggtheme = theme_classic()
  )
  
  p$plot <- p$plot + theme(
    legend.title = element_text(face = "bold", size = 11) 
  )
  p$table <- p$table + theme(axis.title.y = element_blank())
  
  return(p)
}

combine_km <- function(km_obj) {
  
  km_obj$table <- km_obj$table + theme(axis.title.y = element_blank())
  
  (km_obj$plot / km_obj$table) + plot_layout(heights = c(3, 1))
}

pw_acm1 <- combine_km(create_km("Q_acm_m1", "status_acm", "TyHGB"))
pw_acm2 <- combine_km(create_km("Q_acm_m2", "status_acm", "TyHGB_ABSI"))
pw_acm3 <- combine_km(create_km("Q_acm_m3", "status_acm", "TyHGB_WWI"))
pw_acm4 <- combine_km(create_km("Q_acm_m4", "status_acm", "TyHGB_WHtR"))

pw_cvm1 <- combine_km(create_km("Q_cvm_m1", "status_cvm", "TyHGB"))
pw_cvm2 <- combine_km(create_km("Q_cvm_m2", "status_cvm", "TyHGB_ABSI"))
pw_cvm3 <- combine_km(create_km("Q_cvm_m3", "status_cvm", "TyHGB_WWI"))
pw_cvm4 <- combine_km(create_km("Q_cvm_m4", "status_cvm", "TyHGB_WHtR"))

final_plot <- (pw_acm1 | pw_acm2 | pw_acm3 | pw_acm4) / 
  (pw_cvm1 | pw_cvm2 | pw_cvm3 | pw_cvm4)

final_plot
```

## 4. ROC curves at times

```{r}
#| label: fig-roc
#| fig-cap: "Figure 2: Time-dependent ROC curves for mortality prediction at 3, 5, and 10 years"
#| fig-width: 10
#| fig-height: 7

roc_a1 <- timeROC(T=df_final$time_months, delta=df_final$status_acm, marker=df_final$lp_acm_m1, cause=1, weighting="marginal", times=c(36,60,120))
roc_a2 <- timeROC(T=df_final$time_months, delta=df_final$status_acm, marker=df_final$lp_acm_m2, cause=1, weighting="marginal", times=c(36,60,120))
roc_a3 <- timeROC(T=df_final$time_months, delta=df_final$status_acm, marker=df_final$lp_acm_m3, cause=1, weighting="marginal", times=c(36,60,120))
roc_a4 <- timeROC(T=df_final$time_months, delta=df_final$status_acm, marker=df_final$lp_acm_m4, cause=1, weighting="marginal", times=c(36,60,120))


roc_c1 <- timeROC(T=df_final$time_months, delta=df_final$event_cr, marker=df_final$lp_cvm_m1, cause=1, weighting="marginal", times=c(36,60,120))
roc_c2 <- timeROC(T=df_final$time_months, delta=df_final$event_cr, marker=df_final$lp_cvm_m2, cause=1, weighting="marginal", times=c(36,60,120))
roc_c3 <- timeROC(T=df_final$time_months, delta=df_final$event_cr, marker=df_final$lp_cvm_m3, cause=1, weighting="marginal", times=c(36,60,120))
roc_c4 <- timeROC(T=df_final$time_months, delta=df_final$event_cr, marker=df_final$lp_cvm_m4, cause=1, weighting="marginal", times=c(36,60,120))

plot_roc <- function(r1, r2, r3, r4, time_idx, title) {
  
  get_roc_data <- function(roc_obj) {
    n <- names(roc_obj)
    auc_nm <- if("AUC" %in% n) "AUC" else if("AUC_1" %in% n) "AUC_1" else grep("^AUC", n, value=TRUE)[1]
    fp_nm  <- if("FP" %in% n) "FP" else if("FP_1" %in% n) "FP_1" else grep("^FP", n, value=TRUE)[1]
    tp_nm  <- if("TP" %in% n) "TP" else if("TP_1" %in% n) "TP_1" else grep("^TP", n, value=TRUE)[1]
    
    auc_val <- if(!is.null(auc_nm) && !is.na(auc_nm)) roc_obj[[auc_nm]] else rep(NA, 10)
    fp_val  <- if(!is.null(fp_nm) && !is.na(fp_nm)) roc_obj[[fp_nm]] else matrix(NA, nrow=10, ncol=10)
    tp_val  <- if(!is.null(tp_nm) && !is.na(tp_nm)) roc_obj[[tp_nm]] else matrix(NA, nrow=10, ncol=10)
    
    if(is.null(auc_val) || length(auc_val) == 0) auc_val <- rep(NA, 10)
    
    return(list(AUC = auc_val, FP = fp_val, TP = tp_val))
  }
  
  d1 <- get_roc_data(r1); d2 <- get_roc_data(r2); d3 <- get_roc_data(r3); d4 <- get_roc_data(r4)
  
  df_roc <- data.frame(
    FPR = c(d1$FP[,time_idx], d2$FP[,time_idx], d3$FP[,time_idx], d4$FP[,time_idx]),
    TPR = c(d1$TP[,time_idx], d2$TP[,time_idx], d3$TP[,time_idx], d4$TP[,time_idx]),
    Model = c(
      rep(paste0("TyHGB (AUC=", round(d1$AUC[time_idx], 3), ")"), length(d1$FP[,time_idx])),
      rep(paste0("TyHGB+ABSI (AUC=", round(d2$AUC[time_idx], 3), ")"), length(d2$FP[,time_idx])),
      rep(paste0("TyHGB+WWI (AUC=", round(d3$AUC[time_idx], 3), ")"), length(d3$FP[,time_idx])),
      rep(paste0("TyHGB+WHtR (AUC=", round(d4$AUC[time_idx], 3), ")"), length(d4$FP[,time_idx]))
    )
  )
  
  ggplot(df_roc, aes(x = FPR, y = TPR, color = Model)) +
    geom_line(linewidth = 1) + 
    geom_abline(slope = 1, intercept = 0, linetype = "dashed", color = "gray50") +
    scale_color_nejm() + theme_bw() +
    theme(legend.position = c(0.65, 0.22), 
          legend.title = element_blank(),
          legend.background = element_rect(fill = alpha("white", 0.7), color = "black"),
          legend.text = element_text(size = 9, face = "bold"), 
          title = element_text(size = 12, face = "bold")) +
    labs(title = title, x = "1 - Specificity", y = "Sensitivity")
}

hide_x <- theme(axis.title.x = element_blank(), axis.text.x = element_blank(), axis.ticks.x = element_blank())
hide_y <- theme(axis.title.y = element_blank(), axis.text.y = element_blank(), axis.ticks.y = element_blank())

p_roc_a1 <- plot_roc(roc_a1, roc_a2, roc_a3, roc_a4, 1, "ACM - 3 Years") + hide_x
p_roc_a2 <- plot_roc(roc_a1, roc_a2, roc_a3, roc_a4, 2, "ACM - 5 Years") + hide_x + hide_y
p_roc_a3 <- plot_roc(roc_a1, roc_a2, roc_a3, roc_a4, 3, "ACM - 10 Years") + hide_x + hide_y

p_roc_c1 <- plot_roc(roc_c1, roc_c2, roc_c3, roc_c4, 1, "CVM - 3 Years")
p_roc_c2 <- plot_roc(roc_c1, roc_c2, roc_c3, roc_c4, 2, "CVM - 5 Years") + hide_y
p_roc_c3 <- plot_roc(roc_c1, roc_c2, roc_c3, roc_c4, 3, "CVM - 10 Years") + hide_y

final_roc_plot <- (p_roc_a1 | p_roc_a2 | p_roc_a3) / (p_roc_c1 | p_roc_c2 | p_roc_c3)

final_roc_plot
```

## 5. Incremental predictive value

```{r}
# #| label: tbl-incremental
# #| tbl-cap: "Table 3: Incremental predictive value of adding indices to the base model"
# 
# get_c_ci <- function(time_var, event_var, lp_var, weights_var) {
#   c_obj <- concordance(Surv(time_var, event_var) ~ lp_var, weights = weights_var)
#   c_val <- ifelse(c_obj$concordance < 0.5, 1 - c_obj$concordance, c_obj$concordance)
#   se <- sqrt(c_obj$var)
#   lower <- c_val - 1.96 * se
#   upper <- c_val + 1.96 * se
#   return(list(c_str = paste0(sprintf("%.3f", c_val), " (", sprintf("%.3f", lower), "-", sprintf("%.3f", upper), ")"), 
#               c_val = c_val, se = se))
# }
# 
# compare_models <- function(time_var, event_var, event_cr_var, lp_base, lp_new, weights_var, t0, is_cvm) {
#   
#   base_c <- get_c_ci(time_var, event_var, lp_base, weights_var)
#   new_c  <- get_c_ci(time_var, event_var, lp_new, weights_var)
#   
#   if(is_cvm) { 
#     roc_base <- timeROC(T=time_var, delta=event_cr_var, marker=lp_base, cause=1, weighting="marginal", times=t0, iid=TRUE)
#     roc_new  <- timeROC(T=time_var, delta=event_cr_var, marker=lp_new, cause=1, weighting="marginal", times=t0, iid=TRUE)
#   } else {
#     roc_base <- timeROC(T=time_var, delta=event_var, marker=lp_base, cause=1, weighting="marginal", times=t0, iid=TRUE)
#     roc_new  <- timeROC(T=time_var, delta=event_var, marker=lp_new, cause=1, weighting="marginal", times=t0, iid=TRUE)
#   }
#   
#   comp_res <- compare(roc_new, roc_base, adjusted=FALSE)
#   p_delong <- if(!is.null(comp_res$p_values_AUC)) comp_res$p_values_AUC[1] else comp_res$p_values_AUC_1[1]
#   
#   if(is.na(p_delong)) {
#     se_diff <- sqrt(base_c$se^2 + new_c$se^2)
#     if (se_diff == 0) se_diff <- 1e-6 
#     z_stat <- abs(new_c$c_val - base_c$c_val) / se_diff
#     p_delong <- 2 * (1 - pnorm(z_stat))
#   }
#   
#   indata_df <- data.frame(time = time_var, status = event_var)
#   
#   set.seed(123)
#   idi_res <- IDI.INF(indata_df, covs0 = as.matrix(lp_base), covs1 = as.matrix(lp_new), t0 = t0, npert = 100) 
#   out_matrix <- IDI.INF.OUT(idi_res)
#   
#   idi_str <- paste0(sprintf("%.3f", out_matrix[1, 1]), " (", sprintf("%.3f", out_matrix[1, 2]), "-", sprintf("%.3f", out_matrix[1, 3]), ")")
#   idi_p   <- out_matrix[1, 4]
#   
#   nri_str <- paste0(sprintf("%.3f", out_matrix[3, 1]), " (", sprintf("%.3f", out_matrix[3, 2]), "-", sprintf("%.3f", out_matrix[3, 3]), ")")
#   nri_p   <- out_matrix[3, 4]
#   
#   return(list(c_new = new_c$c_str, p_delong = p_delong, nri = nri_str, p_nri = nri_p, idi = idi_str, p_idi = idi_p))
# }
# 
# run_all_comparisons <- function(time_var, event_var, event_cr_var, lp_m1, lp_m2, lp_m3, lp_m4, weights_var, t0 = 120, is_cvm = FALSE, outcome_name) {
#   cat(paste("\n================ KẾT QUẢ CHO:", outcome_name, "================\n"))
#   
#   c_base_str <- get_c_ci(time_var, event_var, lp_m1, weights_var)$c_str
#   cat(sprintf("Mô hình gốc (TyHGB) - C-index: %s\n\n", c_base_str))
#   
#   cat(sprintf("%-12s | %-19s | %-8s | %-19s | %-8s | %-19s | %-8s\n",
#               "Model so sánh", "C-index New (95%CI)", "P DeLong", "Cont. NRI (95%CI)", "P (NRI)", "IDI (95%CI)", "P (IDI)"))
#   cat(strrep("-", 108), "\n")
#   
#   models <- list(
#     list(name = "TyHGB+ABSI", lp = lp_m2),
#     list(name = "TyHGB+WWI",  lp = lp_m3),
#     list(name = "TyHGB+WHtR", lp = lp_m4)
#   )
#   
#   for (m in models) {
#     res <- compare_models(time_var, event_var, event_cr_var, lp_m1, m$lp, weights_var, t0, is_cvm)
#     p_dl_print <- ifelse(res$p_delong < 0.001, "<0.001", sprintf("%.4f", res$p_delong))
#     p_nri_print <- ifelse(res$p_nri < 0.001, "<0.001", sprintf("%.4f", res$p_nri))
#     p_idi_print <- ifelse(res$p_idi < 0.001, "<0.001", sprintf("%.4f", res$p_idi))
#     
#     cat(sprintf("%-12s | %-19s | %-8s | %-19s | %-8s | %-19s | %-8s\n",
#                 m$name, res$c_new, p_dl_print, res$nri, p_nri_print, res$idi, p_idi_print))
#   }
#   cat("\n")
# }
# 
# cat("Đang xử lý Bootstrap 100 vòng. Sẽ mất vài phút...\n")
# 
# run_all_comparisons(
#   time_var     = df_final$time_months, 
#   event_var    = df_final$status_acm, 
#   event_cr_var = df_final$status_acm,     
#   lp_m1        = df_final$lp_acm_m1,      
#   lp_m2        = df_final$lp_acm_m2, 
#   lp_m3        = df_final$lp_acm_m3, 
#   lp_m4        = df_final$lp_acm_m4, 
#   weights_var  = df_final$WTSAF2YR,
#   t0           = 120, 
#   is_cvm       = FALSE, 
#   outcome_name = "ALL-CAUSE MORTALITY (ACM) TẠI 10 NĂM"
# )
# 
# run_all_comparisons(
#   time_var     = df_final$time_months, 
#   event_var    = df_final$status_cvm, 
#   event_cr_var = df_final$event_cr,       
#   lp_m1        = df_final$lp_cvm_m1,      
#   lp_m2        = df_final$lp_cvm_m2, 
#   lp_m3        = df_final$lp_cvm_m3, 
#   lp_m4        = df_final$lp_cvm_m4, 
#   weights_var  = df_final$WTSAF2YR,
#   t0           = 120, 
#   is_cvm       = TRUE, 
#   outcome_name = "CARDIOVASCULAR MORTALITY (CVM) TẠI 10 NĂM"
# )
```

## 6. Dose-response relationship

```{r}
#| label: fig-rcs
#| fig-cap: "Figure 3: Restricted cubic spline (RCS) curves showing the dose-response relationship between TyHGB index and mortality"
#| fig-width: 16
#| fig-height: 7

rcs_vars <- c("time_months", "status_acm", "status_cvm", "TyHGB", 
              "RIDAGEYR", "RIAGENDR", "RIDRETH1", "INDFMPIR", 
              "DMDEDUC2", "DMDMARTL", "diabetes", "hyperten", "cvd_hx", "WTSAF2YR")

df_rcs <- na.omit(df_final[, rcs_vars])
dd <- datadist(df_rcs)
options(datadist = "dd")
covs_str <- "RIDAGEYR + RIAGENDR + RIDRETH1 + INDFMPIR + DMDEDUC2 + DMDMARTL + diabetes + hyperten + cvd_hx"

plot_rcs_exact_style <- function(event_var, title_str) {
  
  form <- as.formula(paste("Surv(time_months, ", event_var, ") ~ rcs(TyHGB, 4) +", covs_str))
  fit <- cph(form, data = df_rcs, x = TRUE, y = TRUE, weights = df_rcs$WTSAF2YR)
  
  anv <- anova(fit)
  p_overall <- anv["TyHGB", "P"]
  p_nonlin  <- anv[" Nonlinear", "P"] 
  
  p_overall_str <- ifelse(p_overall < 0.001, "< 0.001", sprintf("%.3f", p_overall))
  p_nonlin_str  <- ifelse(p_nonlin < 0.001, "< 0.001", sprintf("%.3f", p_nonlin))
  
  knots_array <- fit$Design$parms$TyHGB
  knots_str <- paste(round(knots_array, 2), collapse = ", ")
  
  anno_text <- paste0("P-overall ", ifelse(p_overall < 0.001, "", "= "), p_overall_str, "\n",
                      "P-non-linear ", ifelse(p_nonlin < 0.001, "", "= "), p_nonlin_str, "\n",
                      "Knots: ", knots_str)
  
  pred <- Predict(fit, TyHGB, fun = exp, ref.zero = TRUE)
  df_pred <- as.data.frame(pred)
  
  min_x <- min(df_rcs$TyHGB, na.rm = TRUE)
  max_x <- quantile(df_rcs$TyHGB, 0.99, na.rm = TRUE) 
  
  upper_limit_y <- 1.5
  
  max_dens <- max(density(df_rcs$TyHGB, na.rm = TRUE)$y)
  scale_factor <- max_dens / (upper_limit_y * 0.5) 
  
  df_knots <- data.frame(k = knots_array)
  
  p <- ggplot() +
    geom_histogram(data = df_rcs, aes(x = TyHGB, y = after_stat(density) / scale_factor),
                   bins = 60, fill = "lightblue", color = "white", alpha = 0.9) +

  geom_line(data = df_pred, aes(x = TyHGB, y = lower), 
            color = "black", linewidth = 1.5) +
    geom_line(data = df_pred, aes(x = TyHGB, y = upper), 
              color = "black", linewidth = 1.5) +
    geom_line(data = df_pred, aes(x = TyHGB, y = yhat), 
              color = "red", linewidth = 1) +

  geom_segment(data = df_knots, aes(x = k, xend = k, y = 0, yend = upper_limit_y * 0.02),
               color = "blue", linewidth = 1) +
    
    geom_hline(yintercept = 1, linetype = "dashed", color = "black", linewidth = 0.6) +
    
    annotate("text", x = Inf, y = -Inf, label = anno_text, 
             hjust = 1.05, vjust = -0.5, size = 4.5, color = "black") +
    
    coord_cartesian(xlim = c(min_x, max_x), ylim = c(0, upper_limit_y)) +
    
    scale_y_continuous(
      name = "Hazard Ratio (95% CI)",
      breaks = seq(0, 1.5, by = 0.5), 
      sec.axis = sec_axis(~ . * scale_factor, name = "Probability Density")
    ) +
    
    theme_bw() +
    labs(title = title_str, x = "TyHGB Index") +
    theme(
      plot.title = element_text(size = 14, face = "bold"),
      axis.title.y.left = element_text(size = 12), axis.text.y.left = element_text(size = 11),
      axis.title.y.right = element_text(size = 12), axis.text.y.right = element_text(size = 11),
      axis.title.x = element_text(size = 12), axis.text.x = element_text(size = 11),
      panel.grid.minor = element_blank(),
      panel.grid.major.x = element_line(linetype = "dotted", color = "gray80")
    )
  
  return(p)
}

p_rcs_acm <- plot_rcs_exact_style("status_acm", "")

p_rcs_cvm <- plot_rcs_exact_style("status_cvm", "")

final_rcs_plot <- (p_rcs_acm | p_rcs_cvm) +
  plot_annotation(
    tag_levels = 'A',           
    tag_prefix = '(',          
    tag_suffix = ')'            
  ) &
  theme(plot.tag = element_text(size = 22, face = "bold")) 

final_rcs_plot
```

## 7. Subgroup analyses

```{r}
#| label: fig-subgroup
#| fig-cap: "Figure 4: Subgroup analyses for the association between TyHGB index and mortality risk"
#| fig-width: 22
#| fig-height: 12

df_sub <- df_final %>%
  mutate(
    Age_group = case_when(
      RIDAGEYR < 40 ~ "18-39",
      RIDAGEYR >= 40 & RIDAGEYR < 60 ~ "40-59",
      RIDAGEYR >= 60 ~ ">=60"
    ),
    Gender = ifelse(RIAGENDR == 1, "Male", "Female"),
    PIR_group = case_when(
      INDFMPIR < 1.3 ~ "Low income",
      INDFMPIR >= 1.3 & INDFMPIR <= 3.5 ~ "Medium income",
      INDFMPIR > 3.5 ~ "High income"
    ),
    Obesity = ifelse(BMXBMI >= 30, "Obesity", "Non-Obesity"),
    Diabetes_cat = ifelse(diabetes == 1, "Yes", "No"),
    Hyperten_cat = ifelse(hyperten == 1, "Yes", "No"),
    CVD_cat = ifelse(cvd_hx == 1, "Yes", "No")
  ) %>%
  mutate(
    Age_group = factor(Age_group, levels = c("18-39", "40-59", ">=60")),
    Gender = factor(Gender, levels = c("Male", "Female")),
    PIR_group = factor(PIR_group, levels = c("Low income", "Medium income", "High income")),
    Obesity = factor(Obesity, levels = c("Non-Obesity", "Obesity")),
    Diabetes_cat = factor(Diabetes_cat, levels = c("Yes", "No")),
    Hyperten_cat = factor(Hyperten_cat, levels = c("Yes", "No")),
    CVD_cat = factor(CVD_cat, levels = c("Yes", "No"))
  )

sub_design <- svydesign(id = ~SDMVPSU, strata = ~SDMVSTRA, weights = ~WTSAF2YR, nest = TRUE, data = df_sub)
all_adj_vars <- c("RIDAGEYR", "RIAGENDR", "RIDRETH1", "diabetes", "hyperten")

var_list <- list(
  "Age" = list(col = "Age_group", rm_cov = "RIDAGEYR"),
  "Gender" = list(col = "Gender", rm_cov = "RIAGENDR"),
  "Race" = list(col = "RIDRETH1", rm_cov = "RIDRETH1"),
  "Diabetes" = list(col = "Diabetes_cat", rm_cov = "diabetes"),
  "Hypertension" = list(col = "Hyperten_cat", rm_cov = "hyperten"),
  "Obesity" = list(col = "Obesity", rm_cov = NA) 
)

generate_subgroup_data <- function(event_var) {
  res_list <- list()
  
  for (v_name in names(var_list)) {
    col_name <- var_list[[v_name]]$col
    rm_cov <- var_list[[v_name]]$rm_cov
    
    curr_covs <- if(!is.na(rm_cov)) setdiff(all_adj_vars, rm_cov) else all_adj_vars
    covs_fmla <- paste(curr_covs, collapse = " + ")

    form_inter <- as.formula(paste("Surv(time_months, ", event_var, ") ~ TyHGB *", col_name, "+", covs_fmla))
    fit_inter <- suppressWarnings(tryCatch(svycoxph(form_inter, design = sub_design), error = function(e) NULL))
    
    p_inter <- NA
    if(!is.null(fit_inter)) {
      p_inter <- suppressWarnings(tryCatch({
        regTermTest(fit_inter, as.formula(paste("~ TyHGB:", col_name)))$p[1]
      }, error = function(e) NA))
    }
    p_inter_str <- ifelse(is.na(p_inter), "N/A", ifelse(p_inter < 0.001, "<0.001", sprintf("%.3f", p_inter)))
    
    res_list[[length(res_list) + 1]] <- data.frame(
      Variables = v_name, Subgroups = "", Event = "", ` ` = paste(rep(" ", 30), collapse = ""),
      `HR(95%CI)` = "", `P value` = "", `P for interaction` = p_inter_str,
      est = NA, lower = NA, upper = NA, check.names = FALSE, stringsAsFactors = FALSE
    )

    lvls <- levels(df_sub[[col_name]])
    for (lvl in lvls) {
      ev_count <- sum(df_sub[[col_name]] == lvl & df_sub[[event_var]] == 1, na.rm = TRUE)
      
      d_sub <- suppressMessages(suppressWarnings(subset(sub_design, get(col_name) == lvl)))
      form_sub <- as.formula(paste("Surv(time_months, ", event_var, ") ~ TyHGB +", covs_fmla))
      fit_sub <- suppressWarnings(tryCatch(svycoxph(form_sub, design = d_sub), error = function(e) NULL))
      
      if (!is.null(fit_sub)) {
        sm <- summary(fit_sub)
        hr <- sm$conf.int[1, "exp(coef)"]
        ci_l <- sm$conf.int[1, "lower .95"]
        ci_u <- sm$conf.int[1, "upper .95"]
        pval <- sm$coefficients[1, "Pr(>|z|)"]
        
        if(!is.na(hr) && hr > 100) { hr <- NA; ci_l <- NA; ci_u <- NA }
        
        if(!is.na(hr)) {
          hr_str <- sprintf("%.3f (%.3f, %.3f)", hr, ci_l, ci_u)
          pval_str <- ifelse(pval < 0.001, "<0.001", sprintf("%.3f", pval))
        } else {
          hr_str <- "N/A"; pval_str <- "N/A"
        }
      } else {
        hr <- NA; ci_l <- NA; ci_u <- NA
        hr_str <- "N/A"; pval_str <- "N/A"
      }
      
      res_list[[length(res_list) + 1]] <- data.frame(
        Variables = "", Subgroups = paste0("      ", lvl), Event = as.character(ev_count),
        ` ` = paste(rep(" ", 30), collapse = ""), `HR(95%CI)` = hr_str, `P value` = pval_str, 
        `P for interaction` = "", est = hr, lower = ci_l, upper = ci_u,
        check.names = FALSE, stringsAsFactors = FALSE
      )
    }
  }
  return(do.call(rbind, res_list))
}

draw_forest_plot <- function(dt) {
  
  fill_colors <- c()
  current_color <- "#E8F4F0" 
  for (i in 1:nrow(dt)) {
    if (dt$Variables[i] != "") {
      current_color <- ifelse(current_color == "white", "#E8F4F0", "white")
    }
    fill_colors <- c(fill_colors, current_color)
  }
  
  tm <- forest_theme(
    base_size = 11,
    refline_gp = gpar(col = "black", lty = "dashed"), 
    arrow_type = "closed",
    arrow_label_just = "end",                         
    ci_pch = 15,
    ci_col = "#E64B35", 
    ci_fill = "#E64B35",
    ci_alpha = 1,
    ci_lty = 1,
    ci_lwd = 1.5,
    ci_Theight = 0.2,
    core = list(bg_params = list(fill = fill_colors))
  )
  
  p <- forest(
    data = dt[, c("Variables", "Subgroups", "Event", " ", "HR(95%CI)", "P value", "P for interaction")],
    est = dt$est,
    lower = dt$lower,
    upper = dt$upper,
    ci_column = 4, 
    ref_line = 1,
    arrow_lab = c("Low risk", "High Risk"),
    
    xlim = c(0.7, 1.2), 
    clip = c(0.7, 1.2), 
    ticks_at = c(0.7, 0.9, 1.0, 1.2), 
    theme = tm
  )
  
  p <- edit_plot(p, row = 1:nrow(dt), col = 1, part = "body", gp = gpar(fontface = "bold"))
  
  p <- add_border(p, part = "header", row = 1, where = "top", gp = gpar(lwd = 1.5, col = "black"))
  
  p <- add_border(p, part = "header", row = 1, where = "bottom", gp = gpar(lwd = 1.5, col = "black"))
  
  p <- add_border(p, part = "body", row = nrow(dt), where = "bottom", gp = gpar(lwd = 1.5, col = "black"))
  
  return(p)
}


dt_acm <- generate_subgroup_data("status_acm")
p_acm <- draw_forest_plot(dt_acm)

dt_cvm <- generate_subgroup_data("status_cvm")
p_cvm <- draw_forest_plot(dt_cvm)

final_forest <- wrap_elements(full = p_acm) | wrap_elements(full = p_cvm)
final_forest <- final_forest + 
  plot_annotation(
    tag_levels = 'A',           
    tag_prefix = '(',           
    tag_suffix = ')'            
  ) &
  theme(plot.tag = element_text(size = 22, face = "bold")) 

final_forest
```

## 8. Mediation analysis

```{r}
#| label: fig-mediation
#| fig-cap: "Figure 5: Mediation analysis of the TyHGB index on ACM (A), CVM (B) via uric acid"
#| fig-width: 18
#| fig-height: 8

df_med_clean <- df_final %>% filter(!is.na(LBXSUA))

mediator_var <- "LBXSUA"

med_covs <- c("RIDAGEYR", "RIAGENDR", "RIDRETH1", "diabetes", "hyperten")
med_covs_str <- paste(med_covs, collapse = " + ")

med_design <- svydesign(id = ~SDMVPSU, strata = ~SDMVSTRA, weights = ~WTSAF2YR, nest = TRUE, data = df_med_clean)

draw_mediation_plot <- function(event_var, title_str) {
  
  form_path_a <- as.formula(paste(mediator_var, "~ TyHGB +", med_covs_str))
  fit_path_a <- svyglm(form_path_a, design = med_design, family = gaussian())
  sm_a <- summary(fit_path_a)
  beta_a <- coef(fit_path_a)["TyHGB"]
  se_a <- sm_a$coefficients["TyHGB", "Std. Error"]
  p_a <- sm_a$coefficients["TyHGB", "Pr(>|t|)"]
  
  form_tot <- as.formula(paste("Surv(time_months, ", event_var, ") ~ TyHGB +", med_covs_str))
  fit_tot <- svycoxph(form_tot, design = med_design)
  sm_tot <- summary(fit_tot)
  beta_tot <- coef(fit_tot)["TyHGB"]
  p_tot <- sm_tot$coefficients["TyHGB", "Pr(>|z|)"]
  
  form_dir <- as.formula(paste("Surv(time_months, ", event_var, ") ~ TyHGB +", mediator_var, "+", med_covs_str))
  fit_dir <- svycoxph(form_dir, design = med_design)
  sm_dir <- summary(fit_dir)
  beta_dir <- coef(fit_dir)["TyHGB"]
  p_dir <- sm_dir$coefficients["TyHGB", "Pr(>|z|)"]
  
  med_row <- grep(paste0("^", mediator_var), rownames(sm_dir$conf.int), value = TRUE)[1]
  hr_b <- sm_dir$conf.int[med_row, "exp(coef)"]
  p_b <- sm_dir$coefficients[med_row, "Pr(>|z|)"]
  
  prop_med <- (beta_tot - beta_dir) / beta_tot * 100
  if(is.na(prop_med) || prop_med < 0) prop_med <- 0 
  
  fmt_p <- function(p) ifelse(!is.na(p) & p < 0.001, "P<0.001", sprintf("P=%.3f", p))
  str_tot <- sprintf("%.3f(95%%CI:%.3f~%.3f, %s)", sm_tot$conf.int["TyHGB", 1], sm_tot$conf.int["TyHGB", 3], sm_tot$conf.int["TyHGB", 4], fmt_p(p_tot))
  str_dir <- sprintf("%.3f(95%%CI:%.3f~%.3f, %s)", sm_dir$conf.int["TyHGB", 1], sm_dir$conf.int["TyHGB", 3], sm_dir$conf.int["TyHGB", 4], fmt_p(p_dir))
  str_a <- sprintf("%.3f(95%%CI:%.3f~%.3f, %s)", beta_a, beta_a-1.96*se_a, beta_a+1.96*se_a, fmt_p(p_a))
  str_b <- sprintf("%.3f(95%%CI:%.3f~%.3f, %s)", hr_b, sm_dir$conf.int[med_row, 3], sm_dir$conf.int[med_row, 4], fmt_p(p_b))
  str_prop <- sprintf("%.2f%%", prop_med)
  
  boxes <- data.frame(x = c(1.5, 8.5, 1.5, 8.5, 5), y = c(6, 6, 1, 1, 4),
                      label = c("TyHGB", title_str, "TyHGB", title_str, "Acid Uric"))
  
  p <- ggplot() + theme_void() + coord_cartesian(xlim = c(0, 10), ylim = c(0, 7)) +
    geom_segment(aes(x = 2.5, y = 6, xend = 7.5, yend = 6), arrow = arrow(length = unit(0.3, "cm")), color = "#2CA02C", linewidth = 1) + 
    geom_segment(aes(x = 2.5, y = 1, xend = 7.5, yend = 1), arrow = arrow(length = unit(0.3, "cm")), color = "#2CA02C", linewidth = 1) + 
    geom_segment(aes(x = 2.2, y = 1.5, xend = 4.2, yend = 3.5), arrow = arrow(length = unit(0.3, "cm")), color = "#2CA02C", linewidth = 1) + 
    geom_segment(aes(x = 5.8, y = 3.5, xend = 7.8, yend = 1.5), arrow = arrow(length = unit(0.3, "cm")), color = "#2CA02C", linewidth = 1) + 
    geom_rect(data = boxes, aes(xmin = x - 1, xmax = x + 1, ymin = y - 0.4, ymax = y + 0.4), fill = "#E8F4F0", color = "#4DBBD5", linewidth = 1, rx = 0.2, ry = 0.2) +
    geom_text(data = boxes, aes(x = x, y = y, label = label), size = 5, fontface = "bold") +
    geom_text(aes(x = 5, y = 6.3, label = "Total effect"), fontface = "bold", size = 4) +
    geom_text(aes(x = 5, y = 5.7, label = str_tot), fontface = "bold", size = 3.8) +
    geom_text(aes(x = 5, y = 1.3, label = "Direct effect"), fontface = "bold", size = 4) +
    geom_text(aes(x = 5, y = 0.7, label = str_dir), fontface = "bold", size = 3.8) +
    geom_text(aes(x = 5, y = 2.5, label = "Proportion mediated"), fontface = "bold", size = 4.5) +
    geom_text(aes(x = 5, y = 2.1, label = str_prop), fontface = "bold", size = 5) +
    geom_text(aes(x = 2.7, y = 2.8, label = str_a), fontface = "bold", size = 3.5, angle = 45) +
    geom_text(aes(x = 7.3, y = 2.8, label = str_b), fontface = "bold", size = 3.5, angle = -45)
  
  return(p)
}

p_med_acm <- draw_mediation_plot("status_acm", "All-cause\nmortality")
p_med_cvm <- draw_mediation_plot("status_cvm", "Cardiovascular\nmortality")

final_mediation <- (p_med_acm | p_med_cvm) + 
  plot_annotation(tag_levels = 'A',
                  tag_prefix = '(',
                  tag_suffix = ')') +
  theme(plot.tag = element_text(size = 22, face = "bold"))

final_mediation
```

## 9. Competing Risk Analysis (Fine-Gray Model)

```{r}
#| label: tbl-finegray
#| tbl-cap: "Table 4: Competing risk analysis (Fine-Gray model) for cardiovascular mortality"

# 1. TẠO DATA SẠCH CHO FINE-GRAY (Tránh lỗi rớt data do NA ở các cột không liên quan)
df_fg <- df_final[, c("time_months", "event_cr", "SDMVPSU", "SDMVSTRA", "WTSAF2YR", 
                      "TyHGB_Z", "ABSI_Z", "WWI_Z", "WHtR_Z", 
                      "RIDAGEYR", "RIAGENDR", "RIDRETH1", "diabetes", "hyperten")]

# Khai báo factor chuẩn cho package survival (0 = Censor, 1 = Sự kiện chính, 2 = Cạnh tranh)
df_fg$event_cr_fg <- factor(df_fg$event_cr, levels = c(0, 1, 2), labels = c("censor", "CVM", "Other_Death"))

# 2. XÂY DỰNG DATASET FINE-GRAY (Tạo thêm trọng số fgwt)
fg_data <- finegray(Surv(time_months, event_cr_fg) ~ ., data = df_fg, etype = "CVM")

# Cập nhật svydesign: Nhân trọng số khảo sát (WTSAF2YR) với trọng số Fine-Gray (fgwt)
fg_design <- svydesign(id = ~SDMVPSU, strata = ~SDMVSTRA, weights = ~WTSAF2YR * fgwt, nest = TRUE, data = fg_data)

# 3. THIẾT LẬP VÒNG LẶP (Y HỆT BƯỚC 2)
markers <- c("TyHGB_Z", "ABSI_Z", "WWI_Z", "WHtR_Z")
adj_vars <- "RIDAGEYR + RIAGENDR + RIDRETH1 + diabetes + hyperten"

get_fg_res <- function(marker) {
  
  # A. Đơn biến (Univariate Fine-Gray)
  form_uni <- paste0("Surv(fgstart, fgstop, fgstatus) ~ ", marker)
  fit_uni <- suppressWarnings(svycoxph(as.formula(form_uni), design = fg_design))
  res_uni <- tidy(fit_uni, exponentiate = TRUE, conf.int = TRUE) %>% filter(term == marker)
  
  sHR_uni <- sprintf("%.3f (%.3f-%.3f)", res_uni$estimate, res_uni$conf.low, res_uni$conf.high)
  p_uni <- ifelse(res_uni$p.value < 0.001, "<0.001", sprintf("%.3f", res_uni$p.value))
  
  # B. Đa biến (Multivariate Fine-Gray)
  form_multi <- paste0("Surv(fgstart, fgstop, fgstatus) ~ ", marker, " + ", adj_vars)
  fit_multi <- suppressWarnings(svycoxph(as.formula(form_multi), design = fg_design))
  res_multi <- tidy(fit_multi, exponentiate = TRUE, conf.int = TRUE) %>% filter(term == marker)
  
  sHR_multi <- sprintf("%.3f (%.3f-%.3f)", res_multi$estimate, res_multi$conf.low, res_multi$conf.high)
  p_multi <- ifelse(res_multi$p.value < 0.001, "<0.001", sprintf("%.3f", res_multi$p.value))
  
  return(c(sHR_uni, p_uni, sHR_multi, p_multi))
}

# 4. CHẠY LOOP VÀ GOM KẾT QUẢ VÀO DATA.FRAME
results_fg <- list()

for (marker in markers) {
  res <- get_fg_res(marker)
  
  clean_marker_name <- gsub("_Z", "", marker) # Xóa đuôi _Z cho đẹp khi in ra bảng
  
  results_fg[[length(results_fg) + 1]] <- data.frame(
    Marker = clean_marker_name,
    Uni_sHR = res[1],
    Uni_P = res[2],
    Multi_sHR = res[3],
    Multi_P = res[4],
    stringsAsFactors = FALSE
  )
}

df_fg_final <- bind_rows(results_fg)

# 5. VẼ BẢNG MA TRẬN CHUẨN Q1 BẰNG 'gt'
table_fg_gt <- df_fg_final %>%
  gt() %>%
  tab_spanner(label = md("**Univariate analysis (per 1-SD increase)**"), columns = c(Uni_sHR, Uni_P)) %>%
  tab_spanner(label = md("**Multivariate Analysis (per 1-SD increase)**"), columns = c(Multi_sHR, Multi_P)) %>%
  cols_label(
    Marker = "",
    Uni_sHR = md("**sHR (95% CIs)**"),
    Uni_P = md("**p-value**"),
    Multi_sHR = md("**sHR (95% CIs)**"),
    Multi_P = md("**p-value**")
  ) %>%
  tab_options(
    column_labels.font.weight = "bold",
    table.border.top.color = "black",
    table.border.bottom.color = "black",
    table_body.border.bottom.color = "black"
  ) %>%
  cols_align(align = "left", columns = everything())

# Hiển thị bảng
table_fg_gt
```

## 10. Construction of Nomograms

```{r}
#| label: fig-nomogram
#| fig-cap: "Figure 6: Nomograms predicting 3-, 5-, and 10-year survival probabilities for ACM (A) and CVM (B)"
#| fig-width: 14
#| fig-height: 14

covs_str <- "RIDAGEYR + RIAGENDR + RIDRETH1 + diabetes + hyperten"

model_vars <- c("TyHGB", "RIDAGEYR", "RIAGENDR", "RIDRETH1", "diabetes", "hyperten")
sys_vars <- c("time_months", "status_acm", "status_cvm", "event_cr", "WTSAF2YR")

# 1. Xử lý Data: Lọc, ép kiểu về Data.frame
df_nomo_rms <- df_final %>%
  select(all_of(c(sys_vars, model_vars))) %>% 
  drop_na() %>%
  as.data.frame() # Chống Tibble error

# ------------------------------------------------------------------------------
# QUAN TRỌNG: PHẢI ÉP FACTOR TRƯỚC (Đổi số thành chữ)
# ------------------------------------------------------------------------------
df_nomo_rms$RIAGENDR <- factor(df_nomo_rms$RIAGENDR, levels = c(1, 2), labels = c("Male", "Female"))
df_nomo_rms$diabetes <- factor(df_nomo_rms$diabetes, levels = c(0, 1), labels = c("No", "Yes"))
df_nomo_rms$hyperten <- factor(df_nomo_rms$hyperten, levels = c(0, 1), labels = c("No", "Yes"))
df_nomo_rms$RIDRETH1 <- as.factor(df_nomo_rms$RIDRETH1)

# ------------------------------------------------------------------------------
# RỒI MỚI DÁN LABEL SAU CÙNG (Để không bị hàm factor xóa mất)
# ------------------------------------------------------------------------------
label(df_nomo_rms$TyHGB)    <- "TyHGB Index"
label(df_nomo_rms$RIDAGEYR) <- "Age (years)"
label(df_nomo_rms$RIAGENDR) <- "Gender"
label(df_nomo_rms$RIDRETH1) <- "Race/Ethnicity"
label(df_nomo_rms$diabetes) <- "Diabetes"
label(df_nomo_rms$hyperten) <- "Hypertension"

# 2. Khởi tạo Datadist
dd <- datadist(df_nomo_rms)
options(datadist = "dd")

# ------------------------------------------------------------------------------
# 3. MÔ HÌNH VÀ NOMOGRAM CHO ACM
# ------------------------------------------------------------------------------
form_acm <- as.formula(paste("Surv(time_months, status_acm) ~ TyHGB +", covs_str))
fit_acm_nomo <- cph(form_acm, data = df_nomo_rms, x = TRUE, y = TRUE, surv = TRUE, 
                    time.inc = 120, weights = df_nomo_rms$WTSAF2YR)

surv_acm <- Survival(fit_acm_nomo)
surv3_acm <- function(lp) surv_acm(36, lp)
surv5_acm <- function(lp) surv_acm(60, lp)
surv10_acm <- function(lp) surv_acm(120, lp)

prob_scale_acm <- c(0.95, 0.90, 0.80, 0.70, 0.60, 0.50)
nom_acm <- nomogram(fit_acm_nomo, fun = list(surv3_acm, surv5_acm, surv10_acm),
                    funlabel = c("3-Year ACM Survival", "5-Year ACM Survival", "10-Year ACM Survival"),
                    fun.at = prob_scale_acm, lp = FALSE)

# ------------------------------------------------------------------------------
# 4. MÔ HÌNH VÀ NOMOGRAM CHO CVM (FINE-GRAY)
# ------------------------------------------------------------------------------
df_nomo_rms$event_cr_fg <- factor(df_nomo_rms$event_cr, levels = c(0, 1, 2), labels = c("censor", "CVM", "Other_Death"))
fg_data_nomo <- finegray(Surv(time_months, event_cr_fg) ~ ., data = df_nomo_rms, etype = "CVM")
fg_data_nomo$final_wt <- fg_data_nomo$WTSAF2YR * fg_data_nomo$fgwt

# Gắn lại Label cho tập Fine-Gray vì hàm finegray cũng có tật xóa label
label(fg_data_nomo$TyHGB)    <- "TyHGB Index"
label(fg_data_nomo$RIDAGEYR) <- "Age (years)"
label(fg_data_nomo$RIAGENDR) <- "Gender"
label(fg_data_nomo$RIDRETH1) <- "Race/Ethnicity"
label(fg_data_nomo$diabetes) <- "Diabetes"
label(fg_data_nomo$hyperten) <- "Hypertension"

options(datadist = "dd") 

form_cvm <- as.formula(paste("Surv(fgstart, fgstop, fgstatus) ~ TyHGB +", covs_str))
fit_cvm_nomo <- cph(form_cvm, data = fg_data_nomo, x = TRUE, y = TRUE, surv = TRUE, 
                    time.inc = 120, weights = fg_data_nomo$final_wt)

surv_cvm <- Survival(fit_cvm_nomo)
surv3_cvm <- function(lp) surv_cvm(36, lp)
surv5_cvm <- function(lp) surv_cvm(60, lp)
surv10_cvm <- function(lp) surv_cvm(120, lp)

prob_scale_cvm <- c(0.99, 0.95, 0.90, 0.80, 0.70)
nom_cvm <- nomogram(fit_cvm_nomo, fun = list(surv3_cvm, surv5_cvm, surv10_cvm),
                    funlabel = c("3-Year CVM Survival", "5-Year CVM Survival", "10-Year CVM Survival"),
                    fun.at = prob_scale_cvm, lp = FALSE)

# ------------------------------------------------------------------------------
# 5. GỘP BIỂU ĐỒ LÊN HÌNH
# ------------------------------------------------------------------------------
par(mfrow=c(2,1), mar=c(5, 4, 4, 2) + 0.1)
plot(nom_acm, cex.axis=0.8, xfrac=0.35)
title("A. Nomogram for All-Cause Mortality (ACM)", adj = 0, line = 1, cex.main=1.2)
plot(nom_cvm, cex.axis=0.8, xfrac=0.35)
title("B. Nomogram for Cardiovascular Mortality (CVM) - Fine-Gray Model", adj = 0, line = 1, cex.main=1.2)
```

## 11. Calibration Plots with 1000 Bootstraps

```{r}
#| label: fig-calibration
#| fig-cap: "Figure 7: Calibration plots comparing predicted vs observed 10-years"
#| fig-width: 14
#| fig-height: 6

set.seed(123)
cal_acm <- calibrate(fit_acm_nomo, cmethod='KM', method="crossvalidation", B=10, u=120)

form_cvm_proxy <- as.formula(paste("Surv(time_months, event_cr == 1) ~ TyHGB +", covs_str))
fit_cvm_cal_proxy <- cph(form_cvm_proxy, data = df_nomo_rms, x = TRUE, y = TRUE, surv = TRUE, 
                         time.inc = 120, weights = df_nomo_rms$WTSAF2YR)
cal_cvm <- calibrate(fit_cvm_cal_proxy, cmethod='KM', method="crossvalidation", B=10, u=120)

# 2. Xây dựng hàm "độ" giao diện bằng ggplot2
plot_cal_ggplot <- function(cal_obj, title_str, color_theme, show_ylab = TRUE) {
  
  df_cal <- data.frame(
    pred = cal_obj[, "mean.predicted"],
    obs = cal_obj[, "KM"],
    obs_corr = cal_obj[, "KM.corrected"]
  )
  
  max_val <- max(c(df_cal$pred, df_cal$obs_corr), na.rm = TRUE) + 0.05
  max_val <- min(max_val, 1) 
  
  p <- ggplot(df_cal, aes(x = pred)) +
    # Đường lý tưởng 45 độ (Ideal)
    geom_abline(intercept = 0, slope = 1, linetype = "dashed", color = "gray50", linewidth = 0.8) +
    
    # Đường Apparent (Chưa hiệu chỉnh)
    geom_line(aes(y = obs, color = "Apparent (Original)"), linetype = "dotted", linewidth = 1) +
    geom_point(aes(y = obs, color = "Apparent (Original)"), size = 2) +
    
    # Đường Bias-corrected (Đã hiệu chỉnh qua Cross-validation)
    geom_line(aes(y = obs_corr, color = "Bias-corrected (Cross-validated)"), linewidth = 1.2) +
    geom_point(aes(y = obs_corr, color = "Bias-corrected (Cross-validated)"), size = 1.5) +
    
    # Ép màu chuẩn tạp chí y khoa
    scale_color_manual(name = "Model Performance:", 
                       values = c("Apparent (Original)" = "black", 
                                  "Bias-corrected (Cross-validated)" = color_theme)) +
    coord_cartesian(xlim = c(0, max_val), ylim = c(0, max_val)) +
    scale_x_continuous(breaks = seq(0, 1, by = 0.1)) +
    scale_y_continuous(breaks = seq(0, 1, by = 0.1)) +
    
    # Trang trí Text & Label (Ẩn trục Y nếu show_ylab = FALSE)
    labs(title = title_str, 
         x = "Predicted 10-Year Probability", 
         y = ifelse(show_ylab, "Observed Actual Proportion", "")) +
    theme_classic() + # Chuyển sang theme classic chuẩn Y khoa
    theme(
      plot.title = element_text(face = "bold", size = 14),
      axis.title = element_text(size = 12, face = "bold"),
      axis.text = element_text(size = 11, color = "black"),
      legend.position = c(0.35, 0.85), # Đặt legend góc trên trái
      legend.background = element_rect(fill = alpha("white", 0.9), color = NA),
      legend.text = element_text(size = 10),
      legend.title = element_text(size = 10, face = "bold", color = "gray30")
    )
  
  return(p)
}

# 3. Gọi hàm vẽ và xuất hình (Hình CVM bên phải tắt trục Y đi)
p_cal_acm <- plot_cal_ggplot(cal_acm, "A. 10-Year ACM Calibration", "#E64B35", show_ylab = TRUE) 
p_cal_cvm <- plot_cal_ggplot(cal_cvm, "B. 10-Year CVM Calibration", "#4DBBD5", show_ylab = FALSE) 

# Dùng patchwork ghép 2 hình song song
p_cal_acm | p_cal_cvm
```

## 12. Decision Curve Analysis (DCA)

```{r}
#| label: fig-dca
#| fig-cap: "Figure 8: Decision Curve Analysis (DCA) demonstrating the net benefit of the TyHGB clinical model at 10 years"
#| fig-width: 14
#| fig-height: 6

df_dca <- as.data.frame(df_nomo_rms)

# 1. TỰ TÍNH LẠI XÁC SUẤT TỬ VONG 10 NĂM
surv10_acm_cal <- Survival(fit_acm_nomo)
df_nomo_rms$prob_acm_10 <- 1 - surv10_acm_cal(120, predict(fit_acm_nomo, newdata = df_dca, type="lp"))

surv10_cvm_cal <- Survival(fit_cvm_nomo)
df_nomo_rms$prob_cvm_10 <- 1 - surv10_cvm_cal(120, predict(fit_cvm_nomo, newdata = df_dca, type="lp"))

# 2. XÂY DỰNG HÀM VẼ DCA ĐÃ TỐI ƯU GIAO DIỆN
plot_beautiful_dca <- function(form, event_time, title_str, line_color, var_name) {
  
  lbl_list <- list()
  lbl_list[[var_name]] <- "TyHGB Model"
  
  dca_obj <- dca(form, data = df_nomo_rms, time = event_time, label = lbl_list)
  df <- as_tibble(dca_obj) %>% dplyr::filter(!is.na(net_benefit))
  
  best_strat <- df %>%
    dplyr::group_by(threshold) %>%
    dplyr::slice_max(order_by = net_benefit, n = 1, with_ties = FALSE) %>%
    dplyr::ungroup() %>%
    dplyr::arrange(threshold)
  
  best_strat <- best_strat %>%
    dplyr::mutate(group = cumsum(label != dplyr::lag(label, default = dplyr::first(label))))
  
  regions <- best_strat %>%
    dplyr::group_by(group, label) %>%
    dplyr::summarize(
      x_start = min(threshold),
      x_end = max(threshold),
      x_mid = mean(threshold),
      .groups = "drop"
    ) %>%
    dplyr::filter((x_end - x_start) > 0.02) 
  
  max_nb <- max(df$net_benefit, na.rm = TRUE)
  y_min <- -max_nb * 0.12  
  strip_y <- -max_nb * 0.06 
  
  p <- ggplot() +
    # 1. Dải băng chiến lược tối ưu (Nằm dưới đáy)
    geom_rect(data = regions, 
              aes(xmin = x_start, xmax = x_end, ymin = y_min, ymax = 0, fill = label), 
              alpha = 0.2) +
    
    # 2. Chữ text trong dải băng
    geom_text(data = regions, 
              aes(x = x_mid, y = strip_y, label = label), 
              color = "gray20", size = 3.5, fontface = "bold", vjust = 0.5) +
    
    # 3. VẼ ĐẦY ĐỦ CẢ 3 ĐƯỜNG (Để Legend bắt được "Treat None")
    geom_line(data = df, 
              aes(x = threshold, y = net_benefit, color = label, linetype = label), 
              linewidth = 1.2) +
    
    # 4. Định dạng màu sắc chuẩn cho đủ 3 nhân vật
    scale_color_manual(values = c("Treat All" = "gray50", "Treat None" = "black", "TyHGB Model" = line_color)) +
    scale_fill_manual(values = c("Treat All" = "gray50", "Treat None" = "black", "TyHGB Model" = line_color)) +
    scale_linetype_manual(values = c("Treat All" = "solid", "Treat None" = "solid", "TyHGB Model" = "solid")) +
    
    coord_cartesian(xlim = c(0, max(df$threshold)), ylim = c(y_min, max_nb * 1.05)) +
    scale_x_continuous(labels = scales::percent_format(accuracy = 1), expand = c(0, 0)) +
    scale_y_continuous(expand = c(0, 0)) +
    
    labs(title = title_str, x = "Threshold Probability", y = "Net Benefit") +
    theme_classic() +
    theme(
      plot.title = element_text(face = "bold", size = 14),
      axis.title = element_text(face = "bold", size = 12),
      axis.text = element_text(size = 11, color = "black"),
      axis.text.x = element_text(margin = margin(t = 5)), # Tách số khỏi trục x 1 xíu cho dễ đọc
      legend.position = c(0.85, 0.85),  
      legend.title = element_blank(),
      legend.text = element_text(size = 11, face = "bold"),
      legend.background = element_rect(fill = alpha("white", 0.8), color = "gray80", linewidth = 0.5),
      panel.grid.major.y = element_line(color = "gray90", linetype = "dotted")
    ) +
    guides(fill = "none") 
    
  return(p)
}

# 3. VẼ VÀ GHÉP 2 HÌNH SONG SONG
p_dca_acm <- plot_beautiful_dca(Surv(time_months, status_acm) ~ prob_acm_10, 120, 
                                "A. 10-Year ACM Decision Curve", "#E64B35", "prob_acm_10")

p_dca_cvm <- plot_beautiful_dca(Surv(time_months, event_cr == 1) ~ prob_cvm_10, 120, 
                                "B. 10-Year CVM Decision Curve", "#4DBBD5", "prob_cvm_10")

p_dca_acm | p_dca_cvm
```

## 13. Threshold Effect Analysis (Two-piecewise Cox Model)

```{r}
#| label: tbl-threshold
#| tbl-cap: "Supplemental Table 4: Threshold effect analysis of TyHGB on ACM and CVM using two-piecewise Cox regression"

library(dplyr)
library(survey)
library(survival)
library(gt)

covs_str <- "RIDAGEYR + RIAGENDR + RIDRETH1 + diabetes + hyperten"

# Hàm tìm điểm uốn (Inflection point) tự động
find_inflection <- function(event_var) {
  # Quét từ bách phân vị thứ 15 đến 85 của TyHGB
  search_grid <- seq(quantile(df_final$TyHGB, 0.15), quantile(df_final$TyHGB, 0.85), length.out = 30)
  best_k <- NA; best_ll <- -Inf
  
  for(k in search_grid) {
    d_temp <- df_final %>% 
      mutate(T1 = ifelse(TyHGB < k, TyHGB, k),
             T2 = ifelse(TyHGB >= k, TyHGB - k, 0))
    des <- svydesign(id = ~SDMVPSU, strata = ~SDMVSTRA, weights = ~WTSAF2YR, nest = TRUE, data = d_temp)
    
    form <- as.formula(paste("Surv(time_months, ", event_var, ") ~ T1 + T2 +", covs_str))
    fit <- suppressWarnings(tryCatch(svycoxph(form, design = des), error = function(e) NULL))
    
    if(!is.null(fit) && fit$ll[2] > best_ll) {
      best_ll <- fit$ll[2]
      best_k <- k
    }
  }
  return(best_k)
}

run_piecewise <- function(event_var, outcome_name) {
  k <- find_inflection(event_var)
  
  d_opt <- df_final %>% mutate(T1 = ifelse(TyHGB < k, TyHGB, k), T2 = ifelse(TyHGB >= k, TyHGB - k, 0))
  des_opt <- svydesign(id = ~SDMVPSU, strata = ~SDMVSTRA, weights = ~WTSAF2YR, nest = TRUE, data = d_opt)
  
  # Standard Cox (Linear)
  fit_lin <- svycoxph(as.formula(paste("Surv(time_months, ", event_var, ") ~ TyHGB +", covs_str)), design = des_opt)
  sm_lin <- summary(fit_lin)
  hr_lin <- sprintf("%.3f (%.3f-%.3f)", sm_lin$conf.int["TyHGB", 1], sm_lin$conf.int["TyHGB", 3], sm_lin$conf.int["TyHGB", 4])
  p_lin <- ifelse(sm_lin$coefficients["TyHGB", 5] < 0.001, "<0.001", sprintf("%.3f", sm_lin$coefficients["TyHGB", 5]))
  
  # Two-piecewise Cox
  fit_pw <- svycoxph(as.formula(paste("Surv(time_months, ", event_var, ") ~ T1 + T2 +", covs_str)), design = des_opt)
  sm_pw <- summary(fit_pw)
  
  hr_t1 <- sprintf("%.3f (%.3f-%.3f)", sm_pw$conf.int["T1", 1], sm_pw$conf.int["T1", 3], sm_pw$conf.int["T1", 4])
  p_t1 <- ifelse(sm_pw$coefficients["T1", 5] < 0.001, "<0.001", sprintf("%.3f", sm_pw$coefficients["T1", 5]))
  
  hr_t2 <- sprintf("%.3f (%.3f-%.3f)", sm_pw$conf.int["T2", 1], sm_pw$conf.int["T2", 3], sm_pw$conf.int["T2", 4])
  p_t2 <- ifelse(sm_pw$coefficients["T2", 5] < 0.001, "<0.001", sprintf("%.3f", sm_pw$coefficients["T2", 5]))
  
  # So sánh mô hình bằng Log-likelihood ratio test (P for LRT)
  lrt_p <- 1 - pchisq(2 * (fit_pw$ll[2] - fit_lin$ll[2]), df = 1)
  lrt_p_str <- ifelse(lrt_p < 0.001, "<0.001", sprintf("%.3f", lrt_p))
  
  data.frame(
    Outcome = outcome_name, Model = c("Standard linear model", "Two-piecewise model", 
                                      sprintf("  TyHGB < %.1f (Inflection point)", k),
                                      sprintf("  TyHGB >= %.1f", k), "P for LRT"),
    `HR (95% CI)` = c(hr_lin, "", hr_t1, hr_t2, ""), `P value` = c(p_lin, "", p_t1, p_t2, lrt_p_str),
    check.names = FALSE
  )
}

df_thresh <- bind_rows(run_piecewise("status_acm", "All-Cause Mortality"), run_piecewise("status_cvm", "Cardiovascular Mortality"))

df_thresh %>% gt(groupname_col = "Outcome") %>%
  tab_options(row_group.font.weight = "bold", row_group.background.color = "#E8F4F0") %>%
  cols_align(align = "left", columns = 1)
```

## 14. Detailed Mediation Analysis

```{r}
#| label: tbl-mediation
#| tbl-cap: "Supplemental Table 5: Detailed mediation analysis of TyHGB on ACM and CVM via multiple biomarkers"

med_vars <- c("LBXSUA" = "Uric Acid", "LBXWBCSI" = "WBC Count", "LBXSASSI" = "AST", 
              "LBXSATSI" = "ALT", "LBDSGLSI" = "Fasting Glucose", "LBDHDD" = "HDL-C")
covs_med <- "RIDAGEYR + RIAGENDR + RIDRETH1 + diabetes + hyperten"

run_sobel_med <- function(event_var, outcome_label) {
  res_list <- list()
  for(v in names(med_vars)) {
    d_clean <- df_final[!is.na(df_final[[v]]), ]
    des <- svydesign(id=~SDMVPSU, strata=~SDMVSTRA, weights=~WTSAF2YR, nest=TRUE, data=d_clean)
    
    # Path A: X -> M
    fit_a <- svyglm(as.formula(paste(v, "~ TyHGB +", covs_med)), design = des)
    a <- coef(fit_a)["TyHGB"]; sa <- summary(fit_a)$coef["TyHGB", "Std. Error"]
    
    # Path B & C': X + M -> Y
    fit_b <- svycoxph(as.formula(paste("Surv(time_months,", event_var, ") ~ TyHGB +", v, "+", covs_med)), design = des)
    b <- coef(fit_b)[v]; sb <- summary(fit_b)$coef[v, "Std. Error"]
    c_prime <- coef(fit_b)["TyHGB"]; sc_prime <- summary(fit_b)$coef["TyHGB", "Std. Error"]
    
    # Path C (Total)
    fit_c <- svycoxph(as.formula(paste("Surv(time_months,", event_var, ") ~ TyHGB +", covs_med)), design = des)
    c_tot <- coef(fit_c)["TyHGB"]; sc_tot <- summary(fit_c)$coef["TyHGB", "Std. Error"]
    
    # Sobel Test
    ab <- a * b
    seb <- sqrt(a^2 * sb^2 + b^2 * sa^2)
    z_ab <- ab / seb
    p_ab <- 2 * (1 - pnorm(abs(z_ab)))
    
    prop_med <- (ab / c_tot) * 100
    if(prop_med < 0) prop_med <- 0
    
    fmt <- function(est, se) sprintf("%.3f (%.3f to %.3f)", est, est - 1.96*se, est + 1.96*se)
    fmt_p <- function(p) ifelse(p < 0.001, "<0.001", sprintf("%.3f", p))
    
    res_list[[length(res_list)+1]] <- data.frame(
      Outcome = outcome_label, Mediator = med_vars[v],
      `ACME (95% CI)` = fmt(ab, seb), `P (ACME)` = fmt_p(p_ab),
      `ADE (95% CI)` = fmt(c_prime, sc_prime), `P (ADE)` = fmt_p(summary(fit_b)$coef["TyHGB", 5]),
      `Prop. Mediated (%)` = sprintf("%.2f%%", prop_med), check.names = FALSE
    )
  }
  bind_rows(res_list)
}

df_med_tbl <- bind_rows(run_sobel_med("status_acm", "All-Cause Mortality"), run_sobel_med("status_cvm", "Cardiovascular Mortality"))

df_med_tbl %>% gt(groupname_col = "Outcome") %>%
  tab_options(row_group.font.weight = "bold", row_group.background.color = "#E8F4F0") %>%
  cols_align(align = "center") %>% cols_align(align = "left", columns = 1)
```

## 15. Sensitivity Analysis (Excluding deaths within 24 months)

```{r}
#| label: tbl-sens
#| tbl-cap: "Supplemental Table 6: Association between TyHGB and mortality excluding events within the first 24 months"

# Lọc bỏ tử vong trong 24 tháng đầu
df_sens <- df_final %>% filter(time_months > 24) %>% mutate(TyHGB_Q_num = as.numeric(TyHGB_Q))
des_sens <- svydesign(id = ~SDMVPSU, strata = ~SDMVSTRA, weights = ~WTSAF2YR, nest = TRUE, data = df_sens)

mods <- list("Model 1" = "", "Model 2" = "+ RIDAGEYR + RIAGENDR", "Model 3" = paste("+", covs_str))

run_sens_model <- function(event_var, outcome_name) {
  res_list <- list()
  for(m in names(mods)) {
    # Continuous
    f_cont <- as.formula(paste("Surv(time_months, ", event_var, ") ~ TyHGB", mods[[m]]))
    fit_cont <- svycoxph(f_cont, design = des_sens)
    sm_cont <- summary(fit_cont)
    hr_cont <- sprintf("%.3f (%.3f-%.3f)", sm_cont$conf.int["TyHGB", 1], sm_cont$conf.int["TyHGB", 3], sm_cont$conf.int["TyHGB", 4])
    p_cont <- ifelse(sm_cont$coef["TyHGB", 5] < 0.001, "<0.001", sprintf("%.3f", sm_cont$coef["TyHGB", 5]))
    
    # Quartiles
    f_q <- as.formula(paste("Surv(time_months, ", event_var, ") ~ TyHGB_Q", mods[[m]]))
    fit_q <- svycoxph(f_q, design = des_sens)
    sm_q <- summary(fit_q)
    
    # P for trend
    f_trend <- as.formula(paste("Surv(time_months, ", event_var, ") ~ TyHGB_Q_num", mods[[m]]))
    fit_trend <- svycoxph(f_trend, design = des_sens)
    p_trend <- ifelse(summary(fit_trend)$coef["TyHGB_Q_num", 5] < 0.001, "<0.001", sprintf("%.3f", summary(fit_trend)$coef["TyHGB_Q_num", 5]))
    
    res_list[[length(res_list)+1]] <- data.frame(
      Outcome = outcome_name, TyHGB = c("Continuous", "Q1", "Q2", "Q3", "Q4", "P for trend"),
      Model = m,
      `HR (95% CI)` = c(hr_cont, "Ref.", 
                        sprintf("%.3f (%.3f-%.3f)", sm_q$conf.int["TyHGB_QQ2", 1], sm_q$conf.int["TyHGB_QQ2", 3], sm_q$conf.int["TyHGB_QQ2", 4]),
                        sprintf("%.3f (%.3f-%.3f)", sm_q$conf.int["TyHGB_QQ3", 1], sm_q$conf.int["TyHGB_QQ3", 3], sm_q$conf.int["TyHGB_QQ3", 4]),
                        sprintf("%.3f (%.3f-%.3f)", sm_q$conf.int["TyHGB_QQ4", 1], sm_q$conf.int["TyHGB_QQ4", 3], sm_q$conf.int["TyHGB_QQ4", 4]), ""),
      `P value` = c(p_cont, "-", 
                    sprintf("%.3f", sm_q$coef["TyHGB_QQ2", 5]), sprintf("%.3f", sm_q$coef["TyHGB_QQ3", 5]), sprintf("%.3f", sm_q$coef["TyHGB_QQ4", 5]), p_trend),
      check.names = FALSE
    )
  }
  bind_rows(res_list)
}

df_sens_tbl <- bind_rows(run_sens_model("status_acm", "All-Cause Mortality"), run_sens_model("status_cvm", "Cardiovascular Mortality")) %>%
  tidyr::pivot_wider(names_from = Model, values_from = c(`HR (95% CI)`, `P value`))

df_sens_tbl %>% select(Outcome, TyHGB, contains("Model 1"), contains("Model 2"), contains("Model 3")) %>%
  gt(groupname_col = "Outcome") %>% tab_options(row_group.font.weight = "bold", row_group.background.color = "#F0F0F0")
```

## 16. Multiple Imputation Stability Check

```{r}
#| label: fig-mice-stability
#| fig-cap: "Supplemental Figure: Stability of the TyHGB prognostic value across multiple imputed datasets (m=5)"
#| fig-width: 14
#| fig-height: 6

library(mice)
library(broom)

# Chạy MICE 5 vòng
imp_5 <- mice(df_step1[, vars_to_impute], m = 5, method = 'pmm', seed = 123, printFlag = FALSE)

get_mice_hr <- function(event_var, title) {
  res_df <- data.frame()
  estimates <- c(); vars <- c()
  
  for(i in 1:5) {
    d_i <- df_step1
    d_i[, vars_to_impute] <- complete(imp_5, i)
    d_i <- d_i %>% mutate(
      HSI = 8*(LBXSATSI/LBXSASSI) + BMXBMI + ifelse(RIAGENDR==2,2,0) + ifelse(diabetes==1,2,0),
      steatosis = ifelse(HSI > 36, 1, 0),
      cm_risk = ifelse(BMXBMI>=25 | (RIAGENDR==1 & BMXWAIST>=94) | (RIAGENDR==2 & BMXWAIST>=80) | LBDSGLSI>=5.55 | diabetes==1 | hyperten==1 | LBXTR>=150 | (RIAGENDR==1 & LBDHDD<40) | (RIAGENDR==2 & LBDHDD<50), 1, 0),
      is_masld = ifelse(steatosis==1 & cm_risk==1, 1, 0),
      TyHGB = (LBXTR/LBDHDD) + (0.7*LBDSGLSI) + (0.1*BMXBMI),
      status_acm = ifelse(mortstat==1, 1, 0),
      status_cvm = ifelse(mortstat==1 & ucod_leading %in% c("001","005"), 1, 0),
      time_months = permth_int
    ) %>% filter(is_masld == 1, (is.na(RIDEXPRG) | RIDEXPRG != 1), !is.na(mortstat), !is.na(time_months), eligstat == 1)
    
    d_i$diabetes <- as.numeric(as.character(d_i$diabetes)); d_i$hyperten <- as.numeric(as.character(d_i$hyperten))
    
    des_i <- svydesign(id=~SDMVPSU, strata=~SDMVSTRA, weights=~WTSAF2YR, nest=TRUE, data=d_i)
    fit_i <- svycoxph(as.formula(paste("Surv(time_months, ", event_var, ") ~ TyHGB +", covs_str)), design = des_i)
    
    estimates[i] <- coef(fit_i)["TyHGB"]
    vars[i] <- vcov(fit_i)["TyHGB", "TyHGB"]
    
    res_df <- rbind(res_df, data.frame(Imputation = paste("Imputation", i), HR = exp(estimates[i]), 
                                       LCI = exp(estimates[i] - 1.96*sqrt(vars[i])), UCI = exp(estimates[i] + 1.96*sqrt(vars[i]))))
  }
  
  # Rubin's Rules for Pooling
  Q_bar <- mean(estimates)
  U_bar <- mean(vars)
  B <- var(estimates)
  T_var <- U_bar + (1 + 1/5) * B
  
  res_df <- rbind(res_df, data.frame(Imputation = "Pooled (Rubin's Rules)", HR = exp(Q_bar), 
                                     LCI = exp(Q_bar - 1.96*sqrt(T_var)), UCI = exp(Q_bar + 1.96*sqrt(T_var))))
  res_df$Imputation <- factor(res_df$Imputation, levels = rev(res_df$Imputation))
  
  ggplot(res_df, aes(x = HR, y = Imputation)) +
    geom_vline(xintercept = 1, linetype = "dashed", color = "black") +
    geom_errorbarh(aes(xmin = LCI, xmax = UCI), height = 0.2, color = "#0055A4", linewidth = 1) +
    geom_point(size = 4, color = ifelse(res_df$Imputation == "Pooled (Rubin's Rules)", "#E64B35", "#0055A4"), 
               shape = ifelse(res_df$Imputation == "Pooled (Rubin's Rules)", 18, 16)) +
    theme_classic() + labs(title = title, x = "Hazard Ratio (95% CI)", y = "") +
    theme(plot.title = element_text(face = "bold"), axis.text = element_text(size = 11, color="black", face="bold"))
}

p_mice_acm <- get_mice_hr("status_acm", "A. ACM Models Stability")
p_mice_cvm <- get_mice_hr("status_cvm", "B. CVM Models Stability")

p_mice_acm | p_mice_cvm
```

## 17. E-Value Analysis

```{r}
#| label: fig-evalue
#| fig-cap: "Supplemental Figure: E-value curves representing the minimum strength of an unmeasured confounder required to nullify the TyHGB effect"
#| fig-width: 14
#| fig-height: 6

# Cài package EValue nếu chưa có: install.packages("EValue")
library(EValue)

plot_evalue <- function(event_var, title) {
  # Lấy HR và LCI của mô hình 3
  fit <- svycoxph(as.formula(paste("Surv(time_months, ", event_var, ") ~ TyHGB +", covs_str)), design = masld_design)
  hr <- exp(coef(fit)["TyHGB"])
  lci <- exp(coef(fit)["TyHGB"] - 1.96*sqrt(vcov(fit)["TyHGB", "TyHGB"]))
  
  # Tính E-value
  ev <- evalues.HR(estimate = hr, lo = lci, hi = NA, rare = TRUE)
  e_point <- ev[2, "point"]
  e_lower <- ev[2, "lower"]
  
  # Tạo data mô phỏng đường cong
  rr_ud <- seq(1, max(e_point * 2, 5), length.out = 100)
  rr_eu_point <- (hr * rr_ud - hr) / (rr_ud - hr)
  rr_eu_lower <- (lci * rr_ud - lci) / (rr_ud - lci)
  
  df_curve <- data.frame(rr_ud = rr_ud, rr_eu_point = rr_eu_point, rr_eu_lower = rr_eu_lower) %>%
    filter(rr_eu_point > 1 & rr_eu_point < max(rr_ud) & rr_eu_lower > 1)
  
  ggplot(df_curve, aes(x = rr_ud)) +
    geom_line(aes(y = rr_eu_point, color = "Point Estimate"), linewidth = 1.2) +
    geom_line(aes(y = rr_eu_lower, color = "Lower Confidence Limit"), linewidth = 1.2, linetype = "dashed") +
    geom_point(aes(x = e_point, y = e_point), color = "#E64B35", size = 4) +
    geom_point(aes(x = e_lower, y = e_lower), color = "#0055A4", size = 4) +
    annotate("text", x = e_point + 0.3, y = e_point + 0.3, label = sprintf("E-value = %.2f", e_point), fontface="bold", color="#E64B35") +
    annotate("text", x = e_lower + 0.3, y = e_lower + 0.3, label = sprintf("E-value (LCI) = %.2f", e_lower), fontface="bold", color="#0055A4") +
    scale_color_manual(values = c("Point Estimate" = "#E64B35", "Lower Confidence Limit" = "#0055A4")) +
    coord_cartesian(xlim = c(1, max(df_curve$rr_ud)*0.6), ylim = c(1, max(df_curve$rr_ud)*0.6)) +
    labs(title = title, x = expression(bold(RR[UD])~"(Confounder-Outcome Effect)"), y = expression(bold(RR[EU])~"(Exposure-Confounder Effect)")) +
    theme_bw() + theme(legend.position = c(0.75, 0.85), legend.title = element_blank(), 
                       plot.title = element_text(face="bold"), legend.background = element_rect(color="black"))
}

p_ev_acm <- plot_evalue("status_acm", "A. E-value for All-Cause Mortality")
p_ev_cvm <- plot_evalue("status_cvm", "B. E-value for Cardiovascular Mortality")

p_ev_acm | p_ev_cvm
```

